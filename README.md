...
- После успешных проверок выполняется deploy preview на GitHub Pages для PR (для веток из этого репозитория).
- Если preview deployment недоступен (например, Pages не включен или preview API вернул 404), pipeline не падает: в PR добавляется предупреждение с инструкцией по настройке.
